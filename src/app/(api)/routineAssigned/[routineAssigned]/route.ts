  ...
  try {
    ...
    if (user.role !== "trainer") {
      return NextResponse.json(
        { message: "You must be a trainer to delete assigned routines." },
        { status: 403 }
      );
    }
    ...
    if (originalRoutine.trainer_id.toString() !== user.id) {
      return NextResponse.json(
        { message: "You can only delete routines you have assigned." },
        { status: 403 }
      );
    }
    ...
    await AssignedExercise.deleteMany({
      assigned_routine_id: routineAssigned_id,
    });

    return NextResponse.json(
      { message: "Assigned routine deleted successfully" },
      { status: 200 }
    );
  } catch (error: any) {
    ...
    return new NextResponse(
      "Error deleting assigned routines" + error.message,
      { status: 500 }
    );
  }
}
